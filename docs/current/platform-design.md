...
> This document is the current platform baseline, updated from the original
> v3 design. For exact implemented routes, verification commands, and known
> gaps, read `docs/current/handoff.md` first.
...
DELETE /api/user/keys/:id             # 删除 API 密钥
...
POST   /api/user/subscription         # 购买套餐
