...
ANY    /v1/*                          # OpenAI 格式转发
ANY    /v1/messages                   # Anthropic 格式转发
ANY    /v1beta/*                      # Gemini 格式转发
```
...
客户端可用三种原生格式中的任一种接入 relay，relay 根据入口路径判断客户端格式，再根据上游渠道类型进行格式转换：

```
入口路径          客户端格式          上游格式
/v1/*            OpenAI Chat        → OpenAI(透传) / Anthropic / Gemini
/v1/messages     Anthropic Messages → Anthropic(透传) / OpenAI / Gemini
